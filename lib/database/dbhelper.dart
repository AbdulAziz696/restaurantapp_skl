...
import 'package:restaurantapp_skl/model/meal_item.dart';
...
//7
  ...
  Future<List<Meal>> gets (String category) async {
    ...
    for (int i= 0; i <list.length; i++){
      ...
      favourite.setFavoriteId(list[i]['idMeal']);
  ...
  Future<int?> delete(Meal meals) async{
    ...
    print("favorite data Delete");
    return res;
  }
  Future<bool> isFavourite(String? idMeal) async{
    var dbClient= await db;
    var sql= "SELECT * FROM favorite WHERE idMeal=?";
    var res = await dbClient!.rawQuery(sql,[idMeal]);
